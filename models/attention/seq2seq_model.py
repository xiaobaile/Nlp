...
import models.attention.datautil as data_utils
...
class Seq2SeqModel(object):
    ...
    def step(self, session, encoder_inputs, decoder_inputs, target_weights,
        ...
        # 定义Input feed
        input_feed = {}
        for index in xrange(encoder_size):
            input_feed[self.encoder_inputs[index].name] = encoder_inputs[index]
        for count in xrange(decoder_size):
            input_feed[self.decoder_inputs[count].name] = decoder_inputs[count]
            input_feed[self.target_weights[count].name] = target_weights[count]
        ...
        if not forward_only:
            ...
            for index in xrange(decoder_size):  # Output logits.
                output_feed.append(self.outputs[bucket_id][index])
