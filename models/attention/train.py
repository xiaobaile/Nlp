...
def get_fan_yi_info():
    vocab_en, rev_vocab_en = datautil.initialize_vocabulary(
        os.path.join(datautil.data_dir, datautil.vocabulary_file_en))
    vocab_size_en = len(vocab_en)
    vocab_ch, rev_vocab_ch = datautil.initialize_vocabulary(
        os.path.join(datautil.data_dir, datautil.vocabulary_file_ch))
    vocab_size_ch = len(vocab_ch)

    files_from, _ = datautil.get_raw_file_list(datautil.ids_dir_from)
    files_to, _ = datautil.get_raw_file_list(datautil.ids_dir_to)
    source_train_file_path = files_from[0]
    target_train_file_path = files_to[0]
    return vocab_size_en, vocab_size_ch, rev_vocab_en, rev_vocab_ch, source_train_file_path, target_train_file_path


def main():
    vocab_size_en, vocab_size_ch, rev_vocab_en, rev_vocab_ch, source_train_file_path, target_train_file_path = \
        get_fan_yi_info()
    ...
    with tf.Session() as sess:
        model = create_model(sess, False, vocab_size_en, vocab_size_ch)

        source_test_file_path = source_train_file_path
        target_test_file_path = target_train_file_path

        train_set = read_data(source_train_file_path, target_train_file_path, max_train_data_size)
        test_set = read_data(source_test_file_path, target_test_file_path, max_train_data_size)
        ...
        while True:
            ...
            if current_step % steps_per_checkpoint == 0:
                ...
                for bucket_id in xrange(len(_buckets)):
                    ...
                    input_str = datautil.ids2texts(reversed([en[0] for en in encoder_inputs]), rev_vocab_en)
                    print("输入", input_str)
                    print("输出", datautil.ids2texts([en[0] for en in decoder_inputs], rev_vocab_ch))
                    ...
                    if datautil.EOS_ID in outputs:
                        ...
                        print("结果", datautil.ids2texts(outputs, rev_vocab_ch))
