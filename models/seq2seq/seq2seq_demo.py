...
def do_generate_x_y(batch_size, seq_len):
    batch_x = list()
    batch_y = list()
    for _ in range(batch_size):
        ...
        sin_data = amp_rand * np.sin(np.linspace(
            seq_len / 15.0 * freq_rand * 0.0 * math.pi + offset_rand,
            seq_len / 15.0 * freq_rand * 3.0 * math.pi + offset_rand, seq_len * 2
        ))
        offset_rand = random.random() * 2 * math.pi
        freq_rand = (random.random() - 0.5) / 1.5 * 15 + 0.5
        ...
        batch_x.append(np.array([sig_data[: seq_len]]).T)
        batch_y.append(np.array([sig_data[seq_len:]]).T)
    batch_x = np.array(batch_x).transpose((1, 0, 2))
    batch_y = np.array(batch_y).transpose((1, 0, 2))
    return batch_x, batch_y


def generate_data(is_train, batch_size):
    seq_length = 15
    if is_train:
...
sample_now, sample_f = generate_data(is_train=True, batch_size=3)
...
for i in range(layers_num):
    t_cells.append(tf.contrib.rnn.GRUCell(hidden_dim))
M_cell = tf.contrib.rnn.MultiRNNCell(t_cells)

dec_outputs, dec_memory = tf.contrib.legacy_seq2seq.basic_rnn_seq2seq(encoder_input, decode_input, M_cell)
...
def train_batch(batch_size):
    X, Y = generate_data(is_train=True, batch_size=batch_size)
...
print("Fin. train loss: {}, \tTEST loss: {}".format(train_losses, test_losses))
...
preout = []
X, Y = generate_data(is_train=False, batch_size=nb_predictions)
